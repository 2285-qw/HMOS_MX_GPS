...
struct VideoFrame {
    ...
    // 从原始数据创建
    VideoFrame(const uint8_t *srcData, size_t size, int32_t w, int32_t h, int32_t s, int64_t ts, int32_t fmt)
        : width(w), height(h), stride(s), timestamp(ts), format(fmt) {
        if (srcData && size > 0) {
    ...
    VideoFrame(const VideoFrame &other)
        : data(other.data), width(other.width), height(other.height), stride(other.stride), timestamp(other.timestamp),
          format(other.format) {}

    // 移动构造函数
    VideoFrame(VideoFrame &&other) noexcept
        : data(std::move(other.data)), width(other.width), height(other.height), stride(other.stride),
...
struct CallbackTask {
    ...
    int32_t height;
    int32_t format;
    enum Type { FRAME, ERROR, FORMAT_CHANGED } type;
};
...
public:
    ...
    // 设置回调函数
    void setCallbacks(OnFrameDecodedCallback frameCallback = nullptr, OnErrorCallback errorCallback = nullptr,
                      OnFormatChangedCallback formatCallback = nullptr);

    // 解码一帧数据（异步）
    bool decodeFrame(const uint8_t *data, size_t size, int64_t timestamp, uint32_t flags = AVCODEC_BUFFER_FLAGS_NONE);
...
private:
    ...
    struct PendingInput {
        ...
        PendingInput(const uint8_t *srcData, size_t size, int64_t ts, uint32_t fl) : timestamp(ts), flags(fl) {
            if (srcData && size > 0) {
                data.assign(srcData, srcData + size);
            }
        }
        ...
        PendingInput(PendingInput &&other) noexcept
            : data(std::move(other.data)), timestamp(other.timestamp), flags(other.flags) {}
